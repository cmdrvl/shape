...
if command -v mktemp >/dev/null 2>&1; then
  report_tmp=$(mktemp "${TMPDIR:-/tmp}/shape-ubs-gate-report.XXXXXX")
else
  report_tmp="${TMPDIR:-/tmp}/shape-ubs-gate-$$.json"
  : >"$report_tmp"
fi
if command -v mktemp >/dev/null 2>&1; then
  log_file=$(mktemp "${TMPDIR:-/tmp}/shape-ubs-gate-log.XXXXXX")
else
  log_file="${TMPDIR:-/tmp}/shape-ubs-gate-$$.log"
  : >"$log_file"
fi
